...
def compare_args(node, intdict, classname, funcname):
    if isinstance(node, ast.FunctionDef):
        ...
        if has_varargs == True:
            raise Exception("This code isn't built to support varargs, " + funcname + " has vararg") #Exception : not implemented error --- GOOGLE
        if has_kwargs == True:
...
if __name__ == "__main__":   #if this is the main thing being run
    for fname in interface.keys():   #loop over file names "fname" as keys and "classname" as values in dictionary called "interface" (for fname, classspec)
            ...
            for specfunc in interface[fname][classname].keys():
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef) and node.name == specfunc:
                        ...
                        assert compare_args(node, intdict, classname, specfunc)
                    elif isinstance(node, ast.ClassDef) and node.name == classname:
                        found.update({node.name: node})
                        assert compare_args(node, fname, classname, specfunc)
pass