...
import os
import json
#os.chdir('/usr/local/src/pysmurf/scratch/KIPMs/')
...
def slowInitializeMkidSmurf(S,band,tone_power,resonator_guess_MHz=5400, ff_freq_min=-250,ff_freq_max=250, amp_cut=0.05, grad_descent_averages=100, eta_scan_averages=1000, should_I_plot=True):
    """
    ...
    S.find_freq(band,tone_power=tone_power,\
                start_freq=ff_freq_min,stop_freq=ff_freq_max,make_plot=should_I_plot,show_plot=should_I_plot,\
    ...
    S.plot_tune_summary(band,eta_scan=True,show_plot=should_I_plot)
    #print(f'S.which_on({band})={S.which_on(band)}')
    smurf_freqs = []
    for ch in S.which_on(band):
        smurf_f = get_freq_in_Hz(S, band, ch, print_out=False)
        smurf_freqs.append(smurf_f)
    smurf_freqs = np.array(smurf_freqs)
    kid_channel_idx = np.argmin(np.abs(smurf_freqs-resonator_guess_MHz*1e6))
    resonant_channel = S.which_on(band)[kid_channel_idx] #int(input("(int) self.resonant_channel="))
    print(f'resonant_channel: {resonant_channel}')
    for ch in S.which_on(band):
        #print('turning off non resonant channels...')
        if ch != resonant_channel:
            S.channel_off(band, channel=int(ch))
    #print(f'S.which_on({band})={S.which_on(band)}')
    ff_freq = get_freq_in_Hz(S, band, resonant_channel, print_out=False)
...
def fit_tune_file(tune_dict, plot=True, verbose=False, fit_res_obj=None):
    ...
    fine_pars, fine_errs = finefit(tune_dict['freqs'][i_start:i_end], tune_dict['r'][i_start:i_end], tune_dict['fr'], fn, show_plots=plot, fit_res_obj=fit_res_obj)
    if verbose ==True:
...
"""
def fit_data(tune_file, band, db_offset=1, plot=False,fit_res_obj=None):
    ...
    fit_dict, fine_errs = fit_tune_file(tune_dict, plot=plot, fit_res_obj=fit_res_obj)
...
"""
def takeDebugData_new(S, band, channel, nsamp, plot=False, welch_nperseg=2**18, show_plot=True, channel_mode=0):
    ...
    if isinstance(channel, type(None)):
        ...
        if plot: 
            for c in c_ons:
                i_con=i[:, c]
                q_con=q[:, c]
                iq_on=i_con + 1j * q_con
                if plot: 
                    fs = S.get_channel_frequency_mhz(band) * 1.0E6
                    welch_IQ(iq_on, fs, welch_nperseg, title=filename, show_plot=show_plot)
                    ### save plots 
                    fig = plt.gcf()
            #plt.savefig(f'/data/smurf_data/mkid_multitone_streaming_metadata/_Figs/{filename}_psd', fmt='tiff')
            plt.show()
...
def logDebugData(S, band, reschannel, nsamp, sync, filename, tunefile, cooldown_str, decimation, filter_f3db,channel_mode, char_avgs={}, awg_settings={}, led_settings={}, MEMS_settings={}, rf_settings={}):
...
"""
    Takes pulse data using the AWG.
"""
def takePulseData(S, awg, AWG_settings, LED_settings, res_freq ):
    char_avgs = {}
    ...
    print(f'debug data filename: {filename}')
    logDebugData(S, BAND, channel, iq, sync, filename, S.tune_file, COOLDOWN, char_avgs, awg_settings=AWG_settings, led_settings=LED_settings)
    send_command(awg,'OUTP OFF')  # Enable the output
    print("turned AWG output off") 
    ...
    for ch in S.which_on(BAND):
        S.channel_off(BAND, ch)
    count += 1
    plt.plot(iq.real[0:10000], iq.imag[0:10000])
    i_load, q_load, sync_load = S.decode_single_channel(directory_name+filename+'.dat', swapFdF= False)
    n_subbands = S.get_number_sub_bands()
    digitizer_frequency_mhz = S.get_digitizer_frequency_mhz()
    subband_half_width_mhz = (digitizer_frequency_mhz / n_subbands)
    i = i_load / subband_half_width_mhz
    q = q_load / (-1* subband_half_width_mhz)
        
    plt.plot(i[0:10000], q[0:10000])
    plt.show()
