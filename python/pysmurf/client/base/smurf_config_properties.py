...
class SmurfConfigPropertiesMixin:
    ...
    def ref_phase_delay(self):
        """Total (analog + digital) round-trip delay.

        Gets or sets the total (analog + digital) round-trip delay.
        This is the total time it takes the tone to be generated in
        DSP (=Digital Signal Processing), traverse the synthesis
        filter bank, get generated by the DAC, propagate through the
        external analog RF circuit (e.g. through a cold RF chain
        including MUX chips), get digitized by the ADC, and then
        traverse the analysis filter bank.  This register is named
        `refPhaseDelay` in firmware.

        Unit-less unsigned integer.  Each step is a 2.4 MHz tick.
        ...
        .. warning::
           Because `refPhaseDelay` includes the digital delay, it will
           vary for different firmware versions.

        Examples
        --------
        Since `refPhaseDelay` is measured in 2.4 MHz ticks, if
        `refPhaseDelay` is 6 that corresponds to a time delay of
        6/(2.4 MHz) = 2.5uS.  For example, a 2 microsecond delay for a
        100 kHz phi0 rate corresponds to a ~1 rad phase shift -- so
        it's particularly important to set `refPhaseDelay` correctly
        if running at high phi0 rates (10 kHz or higher).

        Returns
        -------
        int
           The total (analog + digital) round-trip delay.  Unit-less
           unsigned integer.  Each step is a 2.4 MHz tick.  Different
           carrier firmware versions support different ranges.
