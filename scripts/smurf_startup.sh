...
# can't start without a cfg ; exit if one doesn't exist
if [ ! -f "$startup_cfg" ]; then
    echo "$startup_cfg doesn't exist, unable to start smurf." 1>&2
    exit 1
fi
source ${startup_cfg}
...
source smurf_startup_functions
...
echo "tmux new-session -d -s ${tmux_session_name}"
...
### startup in each pysmurf session
if [ ! -z "$script_to_run" ]; then
    echo "-> Done with smurf startup, running script_to_run=${script_to_run} on all slots ..."
    for slot in ${slots[@]}; do    
