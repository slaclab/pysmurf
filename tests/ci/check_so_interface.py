...
def compare_args(node, fname, classname, funcname, type):
    # Check function
    if type == "isfunc":
        spec_args = interface[fname][classname][funcname]["args"]
    ...
    # Check class
        ...
        if found_meths != spec_meths:
            ...
            print(f"Found: {found_meths}")
            print(f"Expected: {spec_meths}")
            return False
    return True
            ...
            for funcname in funcname.keys():
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef) and node.name == funcname:
                        ...
                        type = "isfunc"
                        assert compare_args(node, fname, classname, funcname, type)
