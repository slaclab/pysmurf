...
for (amplitude,uc_att) in amplitudes_and_uc_atts:
    ...
    sys.stdout.write('\rSetting TES bias for group %d to 0 V\033[K before tune.'.format(bias_group,bias_low))
    S.set_tes_bias_bipolar(bias_group,0,do_enable=True)
    ...
    S.log('Slowly shift TES bias voltage to place where we begin.', S.LOG_USER)    
    for b in small_steps_to_starting_bias:
        sys.stdout.write('\rTES bias at {:4.3f} V\033[K'.format(b))
        sys.stdout.flush()
        S.set_tes_bias_bipolar(bias_group,b,do_enable=False)        
        time.sleep(wait_time)        

    ## make sure we start at bias_low
    sys.stdout.write('\rSetting TES ramp bias low at {:4.3f} V\033[K'.format(bias_low))
    S.set_tes_bias_bipolar(bias_group,bias_low,do_enable=False)            
    time.sleep(wait_time)
    
    S.log('Starting to take TES bias ramp with amplitude={} and uc_att={}.'.format(amplitude,uc_att), S.LOG_USER)    
    ...
    S.log('Done taking TES bias ramp with amplitude={} and uc_att={}.'.format(amplitude,uc_att), S.LOG_USER)
...
S.set_tes_bias_bipolar(bias_group,0,do_enable=False)            
