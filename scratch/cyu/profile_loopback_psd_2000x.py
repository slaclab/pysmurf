...
att_ucs = [6] # coarse loop just to see if things are very different
att_dcs = [6] # same
...
nperseg = 2**18
freqs = np.arange(-249.5,250,2) # 2 MHz apart such that there are 250 per band
    ...
    for att_dc in att_dcs:
        for tone_power in tone_powers:
            ...
            for band in bands:
                ...
                chan_array = np.zeros((len(freqs),1)) # initialize this for later
                # turn everything on first
                for fidx in f_idxs:
                    f = freqs[fidx]
                    try:
                        b,chan = S.set_fixed_tone(center_freq_mhz + f, tone_power)
                        chan_array[fidx] = chan
                    except AssertionError:
                        continue

            for band in bands:
                for fidx in f_idxs:
                    i,q,sync = S.take_debug_data(band=band,channel=chan_array[fidx],rf_iq=True,nsamp=nsamp)
            ...
            np.savetxt('freqs_2000x.csv',xvec,delimiter=',')
            np.savetxt('respi_2000x.csv',bandvec_i,delimiter=',')
            np.savetxt('respq_2000x.csv',bandvec_q,delimiter=',')
            plt.xlabel('Frequency [MHz]')
            plt.ylabel('Noise at 30kHz offset [dBc/Hz]')
            plt.title(f'I/Q noise of low+high band AMC, attuc{att_uc} attdc{att_dc}, tonepower {tone_power}')
            plt.savefig(f'scratch/cyu/rsi_figs/profile_manychan/iq_psd_lowband_uc{att_uc}dc{att_dc}_power{tone_power}_offset{off}khz_2000x.png',bbox_inches='tight')
            plt.close()

            for band in bands:
                S.band_off(band) # turn everything back off for the next loop
